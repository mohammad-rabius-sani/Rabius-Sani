...
(function() {
    ...
    window.addEventListener('scroll', () => {
        ...
        mobileNavLinks.forEach(link => {
            ...
            if (link.getAttribute('href').substring(1) === current) {
                ...
                link.classList.add('border-primary-500');
            }
        });
    });
    ...
    const animateProgressBars = () => {
        ...
        progressBars.forEach((bar, index) => {
            ...
            const duration = 1 + (parseFloat(width) / 100);
            bar.style.transition = `width ${duration}s cubic-bezier(0.1, 0.5, 0.5, 1)`;
            
            // Add a slight delay for each subsequent bar
            setTimeout(() => {
                bar.style.width = width;
                
                // Add a pulse effect after the bar reaches its full width
                setTimeout(() => {
                    bar.classList.add('animate-pulse-slow');
                }, duration * 1000);
            }, index * 100);
        });
    };
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function showFormMessage(message, type) {
            ...
            messageElement.className = type === 'success' 
                ? 'mt-4 p-4 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 rounded-lg border border-green-200 dark:border-green-800 flex items-center' 
                : 'mt-4 p-4 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 rounded-lg border border-red-200 dark:border-red-800 flex items-center';
